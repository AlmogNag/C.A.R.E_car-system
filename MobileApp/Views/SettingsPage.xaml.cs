...
namespace CARE_App_Mobile.Views
{
    public partial class SettingsPage : ContentPage
    {
        ...
        {
            // Retrieving the logged-in user with valid default values
            var user = LoginPage.LoggedInUser;

            string name = !string.IsNullOrEmpty(user?.FullName) ? user.FullName : "Guardian";
            string phone = !string.IsNullOrEmpty(user?.Phone) ? user.Phone : (Preferences.Get("SavedPhone", "050-0000000"));

            lblProfileName.Text = name;
            lblProfilePhone.Text = _isHebrew ? $"טלפון: {phone}" : $"Phone: {phone}";
        }
